# thanks with: https://github.com/Valodim/zsh-capture-completion

zsh_complete(){
  if [[ -z $(zmodload | grep zsh/zpty) ]]; then
    zmodload zsh/zpty || { echo 'error: missing module zsh/zpty' >&2; exit 1 }
  fi

  # if zpty exists then delete it
  if [[ ! -z $(zpty | cut --delimiter=' ' --fields 2 | cut --characters 1) ]]; then
    zpty -d z
  fi
